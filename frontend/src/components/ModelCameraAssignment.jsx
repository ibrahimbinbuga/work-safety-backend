...
export function ModelCameraAssignment() {
  ...
  // { "modelId_cameraId": assignmentId } — API'den gelen mevcut assignment id'leri
  const [assignmentIdByModel, setAssignmentIdByModel] = useState({});
    ...
    try {
      ...
      const entries = await Promise.all(
        modelList.map(async (model) => {
          ...
          // priority ve assignment_id bilgilerini topla
          const priorityMap = {};
          const assignmentMap = {};
          data.forEach((camera) => {
            const key = `${model.id}_${camera.id}`;
            priorityMap[key] = camera.priority || 'medium';
            if (camera.assignment_id) {
              assignmentMap[key] = camera.assignment_id;
            }
          });

          return { modelId: model.id, activeCameras, priorityMap, assignmentMap };
        })
      );
