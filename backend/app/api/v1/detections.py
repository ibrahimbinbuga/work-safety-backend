...
async def get_detections(
    ...
    if current_user.role == "admin":
        ...
        result = await db.execute(select(models.Detection).where(models.Detection.company_id == company.id))
...
async def get_violations(
    ...
    if not company and current_user.role != "admin":
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Company not found")
    if current_user.role == "admin":
        result = await db.execute(select(models.Violations).order_by(models.Violations.tarih_saat.desc()))
    else:
...
async def update_violation_status(
    ...
    if current_user.role == "admin":
        result = await db.execute(
            update(models.Violations).where(models.Violations.id == violation_id).values(review_status=review_status)
        )
    else:
        result = await db.execute(
