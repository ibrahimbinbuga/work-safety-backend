...
from app.core.dependencies import get_admin_user, get_current_user, verify_company_access
...
from app.services.camera_service import restart_camera_with_current_models as _restart_camera
...
async def upload_model(
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            session.add(models.ModelMeta(path=relative_path, version=version, description=description, is_active=False))
...
async def get_company_general_models(
    ...
    company_result = await db.execute(select(models.Company).where(func.upper(models.Company.code) == func.upper(company_code)))
...
async def set_company_general_models(
    ...
    if normalized_model_ids:
        valid_result = await db.execute(select(models.ModelMeta.id).where(models.ModelMeta.id.in_(normalized_model_ids)))
        valid_model_ids = set(valid_result.scalars().all())

    existing_result = await db.execute(select(models.CompanyModel).where(models.CompanyModel.company_id == company.id))
    ...
    if removed_ids:
        await db.execute(delete(models.CompanyModelCamera).where((models.CompanyModelCamera.company_id == company.id) & (models.CompanyModelCamera.model_id.in_(removed_ids))))
        await db.execute(delete(models.CompanyModel).where((models.CompanyModel.company_id == company.id) & (models.CompanyModel.model_id.in_(removed_ids))))
...
async def get_company_model_cameras(
    ...
    if not company:
        raise HTTPException(status_code=404, detail="Sirket bulunamadi")
    cameras_result = await db.execute(select(models.Camera).where(models.Camera.company_id == company.id))
    response = []
...
async def set_company_model_cameras(
    ...
    camera_priorities: dict = payload.get("camera_priorities", {})
    ...
    valid_cam_ids_result = await db.execute(select(models.Camera.id).where(models.Camera.company_id == company.id))
    ...
    for cam_id in valid_cam_ids:
        ...
        if assignment:
            ...
            db.add(models.CompanyModelCamera(company_id=company.id, camera_id=cam_id, model_id=int(model_id), is_active=True, priority=new_priority if new_priority and new_priority in PRIORITY_INTERVALS else "medium"))

    await sync_company_model_activation_summary(db, company.id)
    await db.commit()
    await restart_company_cameras(db, company.id)
    return {"status": "success", "model_id": int(model_id), "selected_camera_count": len(normalized_cam_ids)}
...
async def get_company_models(
    ...
    await verify_company_access(current_user, company_code)
    company_result = await db.execute(select(models.Company).where(models.Company.code == company_code))
    company = company_result.scalars().first()
    if not company:
        raise HTTPException(status_code=404, detail="Sirket bulunamadi")
    result = await db.execute(
        select(models.CompanyModel).where(models.CompanyModel.company_id == company.id).join(models.ModelMeta).order_by(models.ModelMeta.version.desc())
    )
    return [
        {
            "id": cm.id,
...
async def assign_model_to_company(
    ...
    existing = await db.execute(
        select(models.CompanyModel).where((models.CompanyModel.company_id == company.id) & (models.CompanyModel.model_id == model_id))
    )
    if existing.scalars().first():
...
async def activate_model_for_company(
    ...
    cm_result = await db.execute(
        select(models.CompanyModel).where((models.CompanyModel.id == company_model_id) & (models.CompanyModel.company_id == company.id))
    )
    ...
    for camera in cameras:
        ...
        if assignment:
            ...
            db.add(models.CompanyModelCamera(company_id=company.id, camera_id=camera.id, model_id=company_model.model_id, is_active=True))
    await sync_company_model_activation_summary(db, company.id)
    await db.commit()
    await restart_company_cameras(db, company.id)
    return {"status": "success"}
...
async def deactivate_model_for_company(
    ...
    await db.execute(
        ...
        .where((models.CompanyModelCamera.company_id == company.id) & (models.CompanyModelCamera.model_id == company_model.model_id))
...
async def update_camera_model_priority(
    ...
    if priority not in PRIORITY_INTERVALS:
        raise HTTPException(status_code=400, detail=f"Gecersiz oncelik. Gecerli degerler: {list(PRIORITY_INTERVALS.keys())}")
    assignment_result = await db.execute(
        select(models.CompanyModelCamera).where((models.CompanyModelCamera.id == assignment_id) & (models.CompanyModelCamera.company_id == company.id))
    )
    ...
    return {"status": "success", "assignment_id": assignment_id, "priority": priority, "interval_seconds": PRIORITY_INTERVALS[priority]}
...
async def detect(
    ...
    try:
        ...
        results = model(img)
        detections = [{"class": model.names[int(box.cls)], "confidence": float(box.conf), "bbox": box.xyxy[0].tolist()} for r in results for box in r.boxes]
        annotated_img = results[0].plot()
        _, buffer = cv2.imencode(".jpg", annotated_img)
        image_base64 = base64.b64encode(buffer).decode()
        return {"status": "success", "detections": len(detections), "objects": detections, "image_base64": image_base64, "processing_time": results[0].speed.get("inference", 0)}
    except Exception as e:
        traceback.print_exc()
