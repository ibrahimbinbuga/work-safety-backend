...
from app.core.dependencies import get_current_user, verify_company_access
from app.db.session import get_db
from app.schemas.auth import TokenData

router = APIRouter()

VIOLATION_TYPE_LABELS = {"head": "No Helmet", "vest": "No Vest", "fallen": "Fall Detected"}
VIOLATION_TYPE_MODELS = {"head": "PPE Model", "vest": "PPE Model", "fallen": "Fall Detection Model"}
_TYPE_TO_MODEL_GROUP = {"head": "PPE Model", "vest": "PPE Model", "fallen": "Fall Detection Model"}


async def _get_company(db: AsyncSession, company_code: str) -> models.Company:
    result = await db.execute(select(models.Company).where(func.upper(models.Company.code) == func.upper(company_code)))
...
def _build_filters(company_id: int, from_date: Optional[date], to_date: Optional[date], violation_types: List[str], review_statuses: List[str]):
    filters = [models.Violations.company_id == company_id]
    if from_date:
        filters.append(models.Violations.tarih_saat >= datetime.combine(from_date, time.min))
    if to_date:
        filters.append(models.Violations.tarih_saat <= datetime.combine(to_date, time.max))
    if violation_types:
...
async def get_active_violation_types(
    ...
    result = await db.execute(
        ...
        .where(models.CompanyModel.company_id == company.id, models.CompanyModel.is_active == True)
    )
    ...
    return {
        "types": active_types,
        "groups": [
            {"label": group_label, "types": [t for t in active_types if _TYPE_TO_MODEL_GROUP.get(t) == group_label]}
            for group_label in dict.fromkeys(_TYPE_TO_MODEL_GROUP[t] for t in active_types if t in _TYPE_TO_MODEL_GROUP)
        ],
    }
...
async def get_report_data(
    ...
    result = await db.execute(select(models.Violations).where(and_(*filters)).order_by(models.Violations.tarih_saat.asc()))
    ...
    for v in violations:
        ...
        if v.tarih_saat:
            by_day[v.tarih_saat.strftime("%Y-%m-%d")] += 1
    ...
    if from_date and to_date:
        ...
        while current <= to_date:
            ...
            daily_data.append({"date": current.strftime("%b %d"), "full_date": key, "violations": by_day.get(key, 0)})
            current += timedelta(days=1)
    else:
        for key in sorted(by_day.keys()):
            dt = datetime.strptime(key, "%Y-%m-%d")
            daily_data.append({"date": dt.strftime("%b %d"), "full_date": key, "violations": by_day[key]})

    violation_distribution = [{"name": VIOLATION_TYPE_LABELS.get(t, t), "value": cnt, "type": t} for t, cnt in sorted(by_type.items(), key=lambda x: -x[1])]
    camera_data = [{"camera": cam, "violations": cnt} for cam, cnt in sorted(by_camera.items(), key=lambda x: -x[1])[:10]]
    return {
        "summary": {
...
async def export_csv(
    ...
    company = await _get_company(db, company_code)
    filters = _build_filters(company.id, from_date, to_date, violation_types, review_statuses)
    result = await db.execute(select(models.Violations).where(and_(*filters)).order_by(models.Violations.tarih_saat.desc()))
    violations = result.scalars().all()
    output = io.StringIO()
    writer = csv.DictWriter(output, fieldnames=["ID", "Type", "Model", "Camera", "Date & Time", "Status", "Worker ID"])
    writer.writeheader()
    for v in violations:
        writer.writerow(
            {
...
async def export_excel(
    ...
    period_str = f"{from_date}  →  {to_date}" if from_date and to_date else ""
